...
/// </para>
/// <para>
/// <b>It heals in pulses, not continuously.</b> <c>healPerPulse</c> every <c>pulseInterval</c>,
...
/// </para>
/// <para>
/// <b>The player is the only thing it heals in V1, and the class says so rather than implying it.</b>
/// It takes <see cref="Health"/> — the player's — and not an <c>EnemySystem</c>. M5-04's Wights are
/// the first allies and are the task that widens this; an enemy-healing zone is an affix (M7-02) and
/// would be a faction on the entry, decided then. Deciding it now would be inventing a faction model
/// for one caster.
/// </para>
/// <para>
...
public sealed class ZoneSystem
{
    ...
    /// </param>
    /// <param name="events">Where <c>ZoneSpawned</c>, <c>ZoneHealed</c> and <c>ZoneExpired</c> go.</param>
    ...
    /// <summary>
    /// Places one where the player is standing and publishes <c>ZoneSpawned</c>.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// <param name="healPerPulse">Hit points one pulse restores. Finite and greater than zero.</param>
    ...
    /// A number is not finite and greater than zero, or <paramref name="now"/> is not finite.
    /// </exception>
    /// <exception cref="InvalidOperationException"><see cref="Capacity"/> zones already stand.</exception>
    public int Spawn(
        float radius,
        float duration,
        float healPerPulse,
        ...
        object source)
    {
        ...
        Positive(healPerPulse, nameof(healPerPulse));
        ...
        _ids[slot] = id;
        _positions[slot] = _blackboard.PlayerPosition;
        _radii[slot] = radius;
        _heals[slot] = healPerPulse;
    ...
    /// <summary>Heals whoever is standing in the zone at <paramref name="index"/>, and says so.</summary>
    /// <remarks>

...
| `RunSession.Tick`: time → ingest → combat → **skills** → enemy behaviours → **projectiles** → (dead? end) → **xp drain** → **director** → **stage flow** → motor → intent | the gun aims at where enemies *were*; the motor turns before it knows its facing | M1-06, M1-08, M2-05, M2-07a, M2-10, M3-01a, M3-06 |
...
| **`SkillTree.Restore(takenNodeIds)` runs before `Health.Restore(hp, shield)`** in `RunSession.Start`'s restore block. The rest of that block — the level tracker, the simulated clock — is order-independent; these two are not | the saved hit points are absolute and are clamped against the **live** maximum, and the tree is what moves that maximum. A `+20 max HP` node replayed after the clamp means a run saved at **150 of 160 comes back at 140 of 160**: the player loses the difference once per resume, silently, in their disfavour, and the only symptom is a bar slightly shorter than the one they put the phone down in front of. `PlayerShield` has the same shape against `ShieldSpec`'s maximum the day a node moves it (M3-11). **This is why M3-03 depends on M3-01b rather than the other way round** — the absolute capture had to exist before there was anything that could move the maximum it is clamped against | **M3-03.** `RunSessionResumeTests.Start_RestoresNodesBeforeHealth` is the row, and it fails with the two lines swapped rather than merely reporting a different number. The restore is silent and gated for `LevelTracker.Restore`'s reasons: nothing may publish before `RunStarted`, and a saved order that breaks the tree's own gating is refused rather than absorbed |

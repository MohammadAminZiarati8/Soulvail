...
analyzers, and the errors are the errors the Editor would give. **There is no `dotnet` SDK here —
only a runtime — but `csc.dll` is framework-dependent, so `dotnet csc.dll` works where
`dotnet build` does not.** The rsp is regenerated by `AssetDatabase.ImportAsset`/`Refresh`, so a

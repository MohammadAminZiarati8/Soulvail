...
passes as of M3-12c, so a failure is the current task's. **A folder passed to `--include` needs its
trailing slash, or the check is green over nothing**: `--include Assets/_Project` matches no file,
prints *Formatted 0 of 0 files* under `-v diag` and exits 0; `--include Assets/_Project/` checks all
415 (M6-11). Files named by path are unaffected. When the scope is a folder, read the *Formatted n of
m* line before believing the exit code. The rsp is regenerated by

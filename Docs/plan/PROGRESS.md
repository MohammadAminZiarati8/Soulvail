...
_Updated: 2026-09-24, as of M6-11b's merge — M6 accepted and archived, and nothing left in front of its tag; M7 not started._
...
| **Last merged task** | **[M6-11b](tasks/M6-11b-a-resume-keeps-the-claiming.md) — a resume keeps the Claiming.** `RunEconomy.Claimed` is saved beside the meter and `Veilrot.Restore` latches on it or on 100, so a Claimed run spent or cleansed below 100 comes back Claimed and draining — v4 re-cut, not bumped. |
| **Next task** | **The `m6` tag, the owner's**; then [M6-11c](tasks/M6-11c-stage-one-without-the-m1-dummies.md), [d](tasks/M6-11d-no-reroll-for-a-finished-tree.md) and [e](tasks/M6-11e-two-rows-that-assert-a-premise.md) in any order, none of them gating it; then **M7-00a**, M7's first spec group, against [its ledger](ROADMAP.md#carry-forward-into-m7). |
| **Verified** | **3 152 EditMode / 0 / 0 and PlayMode 26 / 0 / 0** at M6-11b — an EditMode pass taken *after* PlayMode fails the Editor-clock row, and PlayMode can fail the frame-order flake: both [M7 row 8](ROADMAP.md#carry-forward-into-m7). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built).** |

...
| | |
|---|---|
| **Milestone** | M0 — Walking skeleton (in progress, 5/20) |
| **Last merged task** | [M0-05](tasks/M0-05-world-snapshot.md) — `WorldSnapshot`, `EnemySense`, `Num` vector conversions |
| **In progress** | — |
| **Next task** | [M0-06](tasks/M0-06-intents.md) — `PlayerMoveIntent`, `IIntentSink`, `IntentBuffer` |
| **What works** | Nothing runs yet, but core can now say what happened, be told where things are, and have a run reproduced from a seed. `StateMachine<T>`, two ports — `IDomainEvents`, `IRandom` — and the tick channel's payload `WorldSnapshot`/`EnemySense` exist in `Soulvail.Core`; `DomainEventHub`, `SeededRandom` and the `Num` vector conversions are the Unity side; `RecordingEvents` and `FixedRandom` are the fakes every later core test will assert through. Nothing fills a snapshot or draws a random number yet — the shapes exist so `RunSession` (M0-10) and `SnapshotBuilder` (M0-16) can use them. Four of the five assemblies emit code and the Test Runner lists 51 EditMode tests. `AllocationAssert` is the suite's one way to claim "allocates nothing". VContainer 1.19.0 installed; only `Soulvail.Editor` still has no scripts. |

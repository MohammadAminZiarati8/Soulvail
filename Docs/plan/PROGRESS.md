...
_Updated: 2026-09-25, as of RS-03a's merge — core can hold a class's fire on the move and play a class with no movement skill, and no class does yet; M7's specs stand at two groups of five, nothing of M7 built._
...
| **Last merged task** | **[RS-03a](tasks/RS-03a-holding-fire-on-the-move.md) — holding fire on the move, and a class with no movement skill.** `PlayerCombat.FireWhileMoving` is a `Stat`, and a class seeded at 0 holds fire until it has stopped, drops a draw it moves out of, faces where it runs and publishes `HoldFireChanged`. `MovementSkillKind.None` ignores the press and hides the button. The three shipped classes play as before. RS-00, RS-02a and RS-00b merged as #166–#168 just before it. |
| **Next task** | **RS-02b** (a body per class) and **RS-03b** (the volley, core), which do not depend on each other. **After the Ranger's kit, the owner designs the second new class.** M7-00c's specs are on a pushed branch with no PR opened. **The `m6` tag is still the owner's.** **Two rulings are the owner's before M7-00e**: where the art comes from and where the audio comes from. |
| **Verified** | **RS-03a: 3 216 EditMode — 3 215 passed / 0 failed / 1 inconclusive — and PlayMode 53 / 0 / 0**, +20 and +0, run through `TestRunnerApi` because the CLI's `run_tests` hung ([Traps §3](../Traps.md)). The inconclusive row is by construction: one of the two animator clock rows always is, and which one depends on the Editor's clock ([Traps §3](../Traps.md)). **`RangerSandboxTests.Loop_ShootsOnlyStandingStill` can fail a pass on unchanged code** ([Traps §8](../Traps.md)). **A PlayMode pass also overwrites the real `run.json`** ([parking lot](ROADMAP.md#parking-lot)). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built).** |

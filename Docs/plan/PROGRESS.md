...
| | |
|---|---|
| **Milestone** | M0 — Walking skeleton (in progress, 7/20) |
| **Last merged task** | [M0-07](tasks/M0-07-player-motor.md) — `MovementSpec`, `PlayerMotor`: accel/decel, no inertia, facing |
| **In progress** | — |
| **Next task** | [M0-08](tasks/M0-08-content-catalog.md) — `ContentId`, `LocKey`, `CharacterSpec`, `ContentCatalog` |
| **What works** | Every wire of ADR-0003's boundary exists in shape, and as of M0-07 there is something behind it: core can say what happened, be told where things are, be told what to do with the body, have a run reproduced from a seed, and now **decide how the player moves**. `PlayerMotor` + `MovementSpec` turn a stick vector into a velocity and a facing — the first game logic in the project, and the first thing whose tests are about the game rather than about plumbing. Around it: `StateMachine<T>`, three ports — `IDomainEvents`, `IRandom`, `IIntentSink` — and the payloads `WorldSnapshot`/`EnemySense` (in) and `PlayerMoveIntent` (out) in `Soulvail.Core`; `DomainEventHub`, `SeededRandom`, `IntentBuffer` and the `Num` vector conversions on the Unity side; `RecordingEvents` and `FixedRandom` as the fakes every later core test asserts through. Still nothing *runs*: nothing fills a snapshot, draws a random number, writes an intent, or calls `PlayerMotor.Tick` — the shapes exist so `RunSession` (M0-10) and `SnapshotBuilder`/`PlayerView` (M0-16) can wire them together. Four of the five assemblies emit code and the Test Runner lists 76 EditMode tests. `AllocationAssert` is the suite's one way to claim "allocates nothing". VContainer 1.19.0 installed; only `Soulvail.Editor` still has no scripts. |

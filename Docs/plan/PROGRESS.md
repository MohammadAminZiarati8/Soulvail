...
| **Last merged task** | **[RS-02a](tasks/RS-02a-a-playable-ranger.md) — a playable Ranger, in a sandbox.** KayKit's Ranger with a bow in `handslot.l` runs, and stops to shoot at dummies in `Scenes/RangerShowcase.unity`, on the Run scene's stick and camera, with core's `PlayerMotor`, `Targeter` and `Weapon` deciding. **It shoots standing still, by the owner's ruling**, and shooting on the move, meant to be a skill, is the scope's *Shoot While Moving* switch. `AC_Ranger`'s arms are a masked layer, and its legs strafe for that switch. Built at the owner's request, ahead of the new model (RS-01a–c), which will wear the same animator. |

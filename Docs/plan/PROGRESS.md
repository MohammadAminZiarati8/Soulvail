...
_Updated: 2026-09-26, as of RS-03f's merge — the Descend row taps Descend, and it is green with a save on disk and without one. M7's third spec group is written, two to go, nothing of M7 built._
...
| **Last merged task** | **[RS-03f](tasks/RS-03f-the-descend-row.md) — the Descend row taps Descend.** `BootSmokeTests`' Descend row tapped the Menu's first button, which is Continue whenever a run is on disk, so it was green only with a save. It now taps Descend and then a class card, each read from the field that wires it, and `ResumeFlowTests` holds Continue's guard. No production code. **RS-03e's play list is still the owner's**: the roll kept or switched off, and each number with where it lives ([its manual steps](tasks/RS-03e-the-rangers-acceptance.md#manual-verification--what-the-owner-plays)). |
| **Next task** | **M7-00d** — specs for the eighty-one nodes — on M7's track, and on the Ranger's **RS-03g** (the PlayMode suite writes the owner's real `run.json`), then **RS-04**, the showcase scene; the two tracks do not depend on each other. **After the Ranger's kit, the owner designs the second new class.** **The `m6` tag is still the owner's.** **One ruling is the owner's before M7-00e: where the audio comes from.** Art was ruled on 2026-09-25: the KayKit packs in the project, plus Gemini-made models skinned onto `Rig_Medium`, with weapons as separate props. |
| **Verified** | **RS-03f: 3 270 EditMode — 3 269 passed / 0 failed / 1 inconclusive — and PlayMode 57, 56 / 1**, each with a `run.json` on disk and without one, run through `TestRunnerApi` because the CLI's `run_tests` hung at RS-03a ([Traps §3](../Traps.md)). **The red row is `RangerSandboxTests.Loop_ShootsOnlyStandingStill`, which fails full passes while Unity is not the active application, on `dev`'s code as well: queue the pass with Unity active, and bisect against `dev` if it still fails** ([Traps §8](../Traps.md)). **A save-store row can fail a full pass on a Windows file lock; re-run it alone** ([Traps §7](../Traps.md)). **After a prefab is re-saved in a live domain, `RangerTests.Ranger_WearsItsBodyAndFliesArrows` fails until a script reload** ([Traps §5](../Traps.md)). The inconclusive row is by construction: one of the two animator clock rows always is, and which one depends on the Editor's clock ([Traps §3](../Traps.md)). **A PlayMode pass also overwrites the real `run.json`**, so back it up first (RS-03g). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built), and the Ranger's to stage 16 are [RS-03e's](tasks/RS-03e-the-rangers-acceptance.md#as-built).** |

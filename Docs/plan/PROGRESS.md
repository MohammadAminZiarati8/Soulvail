...
_Updated: 2026-09-26, as of RS-03e's merge — the Ranger is accepted on the instrument: a Husk takes 3 to 5 arrows at every stage from 1 to 16. M7's third spec group is written, two to go, nothing of M7 built._
...
| **Last merged task** | **[RS-03e](tasks/RS-03e-the-rangers-acceptance.md) — the Ranger's acceptance.** M6-11's hits-to-kill probe, rebuilt with a temporary PlayMode driver and reverted, played one Ranger run to stage 17. Every stage from 1 to 16 sits inside GD §12.4's 3–5, and no number moved. The spec's manual steps are the owner's play list: the roll kept or switched off, and each number with where it lives. |
| **Next task** | **M7-00d** — specs for the eighty-one nodes — on M7's track, and on the Ranger's **RS-03f** (a PlayMode row that is green only with a save on disk), then **RS-04**, the showcase scene; the two tracks do not depend on each other. **After the Ranger's kit, the owner designs the second new class.** **The `m6` tag is still the owner's.** **One ruling is the owner's before M7-00e: where the audio comes from.** Art was ruled on 2026-09-25: the KayKit packs in the project, plus Gemini-made models skinned onto `Rig_Medium`, with weapons as separate props. |
| **Verified** | **RS-03e: 3 269 EditMode — 3 268 passed / 0 failed / 1 inconclusive — and PlayMode 57 / 0 / 0**, run through `TestRunnerApi` because the CLI's `run_tests` hung at RS-03a ([Traps §3](../Traps.md)). **`BootSmokeTests.Descend_StartsARun_AndRefusesASecondTap` fails whenever no `run.json` is on disk**, because it taps Continue rather than Descend; that is RS-03f, so a red pass on a machine with no save is that row, not a regression. **A save-store row can fail a full pass on a Windows file lock; re-run it alone** ([Traps §7](../Traps.md)). **After a prefab is re-saved in a live domain, `RangerTests.Ranger_WearsItsBodyAndFliesArrows` fails until a script reload** ([Traps §5](../Traps.md)). The inconclusive row is by construction: one of the two animator clock rows always is, and which one depends on the Editor's clock ([Traps §3](../Traps.md)). **`RangerSandboxTests.Loop_ShootsOnlyStandingStill` can fail on unchanged code, alone as well as in a pass; bisect against `dev`** ([Traps §8](../Traps.md)). **A PlayMode pass also overwrites the real `run.json`** ([parking lot](ROADMAP.md#parking-lot)). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built), and the Ranger's to stage 16 are [RS-03e's](tasks/RS-03e-the-rangers-acceptance.md#as-built).** |

...
_Updated: 2026-09-25, as of M7-00c's merge — M7's third spec group written, two to go, nothing of M7 built. Beside it, core can hold a class's fire on the move and play a class with no movement skill (RS-03a), and no class does yet._
...
| **Last merged task** | **[M7-00c](ROADMAP.md#m7--content-pass) — specs for the Choirmother.** Four specs where the table predicted one title: [the ring](tasks/M7-03a-the-ring.md), [the singer](tasks/M7-03b-the-choirmother.md), [what both look like](tasks/M7-03c-a-song-you-can-see.md) and [the roster, built last](tasks/M7-03d-the-choirmother-at-stage-ten.md). Just before it, [RS-03a](tasks/RS-03a-holding-fire-on-the-move.md) made `PlayerCombat.FireWhileMoving` a `Stat` that holds a class's fire until it stops, and added `MovementSkillKind.None`. |
| **Next task** | **M7-00d** — specs for the eighty-one nodes — on M7's track, and **RS-02b** (a body per class) or **RS-03b** (the volley, core) on the Ranger's; none of the three depends on another. **After the Ranger's kit, the owner designs the second new class.** **The `m6` tag is still the owner's.** **One ruling is the owner's before M7-00e: where the audio comes from.** Art was ruled on 2026-09-25: the KayKit packs in the project, plus Gemini-made models skinned onto `Rig_Medium`, with weapons as separate props. |
| **Verified** | **Docs only at M7-00c — no C#, no compile, no suite.** The baseline is **RS-03a's 3 216 EditMode — 3 215 passed / 0 failed / 1 inconclusive — and PlayMode 53 / 0 / 0**, run through `TestRunnerApi` because the CLI's `run_tests` hung ([Traps §3](../Traps.md)). The inconclusive row is by construction: one of the two animator clock rows always is, and which one depends on the Editor's clock ([Traps §3](../Traps.md)). **`RangerSandboxTests.Loop_ShootsOnlyStandingStill` can fail a pass on unchanged code** ([Traps §8](../Traps.md)). **A PlayMode pass also overwrites the real `run.json`** ([parking lot](ROADMAP.md#parking-lot)). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built).** |

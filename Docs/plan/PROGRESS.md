...
_Updated: 2026-09-25, as of M7-00c's merge — M7's third spec group written, two to go; nothing of M7 built._
...
| **Last merged task** | **[M7-00c](ROADMAP.md#m7--content-pass) — specs for the Choirmother.** Four specs where the table predicted one title: [the ring](tasks/M7-03a-the-ring.md), [the singer](tasks/M7-03b-the-choirmother.md), [what both look like](tasks/M7-03c-a-song-you-can-see.md) and [the roster, built last](tasks/M7-03d-the-choirmother-at-stage-ten.md). |
| **Next task** | **M7-00d** — specs for the eighty-one nodes. **The `m6` tag is still the owner's.** **One ruling is the owner's before M7-00e: where the audio comes from.** Art was ruled on 2026-09-25: the KayKit packs in the project, plus Gemini-made models skinned onto `Rig_Medium`, with weapons as separate props. |
| **Verified** | **Docs only at M7-00c — no C#, no compile, no suite.** The baseline stays **3 160 EditMode — 3 159 passed / 0 failed / 1 inconclusive — and PlayMode 25 / 0 / 0 on ten passes of ten** from M6-11e. The inconclusive row is by construction: one of the two animator clock rows always is, and which one depends on the Editor's clock ([Traps §3](../Traps.md)). **A PlayMode pass also overwrites the real `run.json`** ([parking lot](ROADMAP.md#parking-lot)). The chain of counts is each Log entry's **Verified** line ([M6's](archive/PROGRESS-M6.md)). **The played numbers — hits to kill at seven depths for three classes, bodies, frame times, the economy, the Claiming — are [M6-11's *As built*](tasks/M6-11-acceptance-and-tag.md#as-built).** |
...
**Verified:** docs only — no C#, no compile, no suite; the baseline stays M6-11e's **3 160 EditMode (3 159 / 0 / 1 inconclusive)** and **PlayMode 25 / 0 / 0**. Every member, call-site count, asset field and arena position a rule names was read or grepped first. A second reader then checked the four against the code and found about thirty defects, all fixed: asset rows filed under Tests.Core, a second writer on `IsVulnerable` that M7-02c rule 7 forbids, a fault that asked physics of a prefab outside a scene, and 25 rays for 2° gaps. Branched from `dev` at #164, M7-00b's merge, opened and merged on the owner's green light.

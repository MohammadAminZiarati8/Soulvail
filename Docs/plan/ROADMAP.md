...
| **M6** | **Systems complete** | Sanctum shop, Veilrot + Pacts + Claiming, Ordeals, Emberwright, unlocks, localisation tables | 16 |
...
into thirteen specs and a table of fifteen rows; M6's **eleven** titles count to **sixteen rows and
will count higher**, so the groups are themed rather than halved:
...
- **M6-00a — the economy, in core.** [M6-01a](tasks/M6-01a-essence-wallet-and-drops.md),
  ...
  **The seam is the assembly**, which is M5-00a's own: that group's five specs were all
  ...
  GD §10, §13.3 and §15.
- **M6-00b — what the player sees, and what tempts them.** M6-03's screen, M6-05's Pacts and
  M6-06's Ordeals. **M6-05 is predicted to split** at M5-06a/b's seam — *what a Pact is* against
  *the offer that rolls one* — because roughly 1.8× a clean node means scaling an **open set** of
  effect primitives, which is a question about `IEffect` rather than about an offer; the counting is
  that group's.
- **M6-00c — a third class, the meta layer, and the close.** M6-07, M6-08, M6-09, M6-10, M6-11.
  **It inherits a concentration it must split**, the way M5-00a handed one to M5-00b: M6-07's Blink
  leaves a fire pool and `ZoneSystem` heals the player and nothing else, so *"Emberwright spec,
  Cinder Orb, Blink, Kindling"* is very unlikely to be one task.
...
| M6-03 | Sanctum screen | — | M6-02b | ☐ |
| M6-05 | Pact node variants in `OfferGenerator` (GD §13.2) | — | M6-04 | ☐ |
| M6-06 | Ordeals (GD §13.4) | — | M6-04, M6-02b | ☐ |
| M6-07 | Emberwright spec, Cinder Orb, Blink, Kindling | — | M6-04 | ☐ |
| M6-08 | Emberwright tree v1 + actives | — | M6-07 | ☐ |
| M6-09 | Class unlocks: Shards + achievements — the profile's one bump | — | M6-07 | ☐ |
| M6-10 | The **rest** of localisation: locale selection, the other languages' tables, and every screen M3 did not build — `ILocalizer`, `TableLocalizer` and one English table land early at [M3-14a](tasks/M3-14a-localizer-and-english-table.md), by the owner's M3-00d ruling | — | M6-09 | ☐ |
| M6-11 | M6 acceptance, tag `m6` | — | everything | ☐ |
...
| 7 | **Localisation's last third, carried from [M5 row 5(iii)](#carry-forward-into-m5).** Parts (i) and (ii) discharged at [M5-06b](tasks/M5-06b-gravecaller-tree-v1.md) and [M5-02](tasks/M5-02-gravecaller-and-bone-bolt.md). What remains is locale selection, the other languages' tables and every screen M3 did not build — and M5 added four screens to that list (class select, the splash's two pages, and the tree screen's fourth column when it gets one). **M6 adds to it faster than any milestone so far, and the list is written now rather than reconstructed at M6-11:** the Sanctum's four service rows with their prices and their three refusal reasons (M6-03), a Veilrot readout and whatever names the Claiming (M6-04/M6-03), a Pact frame's label (M6-05), six Ordeal names and descriptions (M6-06), the Emberwright's name, description and **twelve** node name/description pairs (M6-07/08), and every locked-class string the unlock gate needs (M6-09). **Locale selection is also a `PlayerProfile` field**, which is why M6-09 is the profile's one bump and M6-10 fills the field rather than bumping again — [M6-01b](tasks/M6-01b-save-format-v4.md)'s argument, one format over. | **M6-10**, which the ROADMAP already holds as a title and which M6-00c specs | **Low now, medium at M6-11.** Every screen shipped before it lands is one more to sweep, and the sweep is the cheap part — the tables are not. |
...
- **`PaletteTests.Palette_HasTheColoursNobodyReadsYet` has been false since M4-06, and it is green.**
  Found at M6-00a. That row asserts *"two reserved colours ship with no reader at all"* and checks
  it against a hand-kept `Readers` list of nine types; `RunEndPresenter` is not on the list and has
  read `Palette.Essence` since [M4-06](tasks/M4-06-run-end-screen.md) — whose own remarks say so in
  as many words, *"`Palette.Essence`'s first reader"* — while `Palette.Essence`'s summary still reads
  *"No reader yet (M6)"*. Three places, two of them wrong, and nothing noticed because the list is
  hand-kept rather than a sweep. Costs nothing today: the claim the row still makes truthfully is
  about `Palette.Veilrot`. Promoted by **M6-03**, the next task to draw an Essence figure, which has
  to correct the summary and narrow the row rather than add to it.

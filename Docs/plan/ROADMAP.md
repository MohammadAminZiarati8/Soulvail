...
| [M2-12a](tasks/M2-12a-threat-arrows-and-held-focus.md) | Screen-edge threat arrows, and a focus that reads as held | M | 07b | ☐ **built, unmerged** — EditMode 911 green; two `FrameOrderTests` `ConeReport` assertions red on a physics-sync question that is [the owner's call](tasks/M2-12a-threat-arrows-and-held-focus.md#open-two-playmode-rows-and-a-question-for-the-owner). The box is ticked when that is settled. |

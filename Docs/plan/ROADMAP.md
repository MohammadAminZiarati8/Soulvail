...
**What M5's specs must absorb.** Opened at [M4-07](tasks/M4-07-acceptance-and-tag.md) on [M2's terms](#carry-forward-into-m2), unchanged through four milestones: every row names the task that must deal with it, and **a row leaves only when its owner's *As built* says it is answered.** Four rows are carried from [M4's table](#carry-forward-into-m4), one is M4 row 6 redistributed, and two are M4-07's own checklist misses. Ranked by what it costs to fix later rather than now.
...
| 7 | **A boss stage completes when the boss is down, whatever its adds are doing** — so an add can hit the player through `Clear` and `Gate`. Found at [M4-01b](tasks/M4-01b-boss-agent-framework.md), which handed it to *"M4-02 or M4-03 to revisit with the fight in front of them"*; neither did, and neither was wrong to skip it, because **nobody has played the fight**. `SpawnDirector.IsStageComplete` returns `_bossCleared` on a boss stage — the same property M4-05a derives the payout from — so the fix is not free: requiring the adds cleared too changes both the pacing of a boss stage and the meaning of the property the payout reads. **This is a mechanism, not a number**, and it is the kind of thing a played run decides. | **M5-00a** to rule — fix, park, or hand to the first playtest that is actually hit through a gate | **Low today, medium at M7.** One boss authors adds; a second (M7-03) and Elites (M7-02) make it a rule rather than an instance. |

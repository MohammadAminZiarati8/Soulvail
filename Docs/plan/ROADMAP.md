...
| **M7** | **Content pass** | Full V1 roster, Elites/affixes, Choirmother, all 81 nodes, both biomes' art, audio — **specs being written, four groups of five done** | 34 (25 counted) |
...
**M7 opens with five spec groups, and the first four are counted.** M6's three groups turned eleven
...
- **M7-00d — the eighty-one nodes.** [M7-04a](tasks/M7-04a-a-pact-on-every-node.md) to
  [M7-04k](tasks/M7-04k-a-full-tree-still-tempts.md). **One title became eleven, split by the kind of
  work.**
  - **A door**: a Pact on every node, the Actives included.
  - **Three verbs**: a burst, a buff that runs out, and an Upgrade that adds to a cast.
  - **Three sets of Keystone mechanisms**, one per class.
  - **Three trees of twenty-seven**, authored as data over all of the above.
  - **The offer past a full tree.**

  **What counting found:**
  - **A saved take order is gated.** `SkillTree.Restore` enforces the tiers, and a refused resume
    strands the run with no message, so every shipped node keeps its id, branch and tier. That is why
    Restless Dead stays in Rot.
  - **No sweep but `SkillTree`'s reads a Pact's effects.**
  - **Nothing hurts a Wight**, so The Host's drawback and Knitted Bone did nothing.
  - **The zone table throws on a ninth zone.**
  - **The swing is drawn at a fixed 60°**, and **the Banish list stops at twelve**.
  - ***Overflow* is already the level-up toast's word**, so the Emberwright's Keystone is **Surge**.

  **Row 10 is answered in full**: 72 of 81 nodes carry a Pact, an Active can, and a full tree still
  offers one (M7-04k).
...
roster is the last of them, so no merged build rosters a song that nothing draws. **M7-00d's eleven
are in ID order, and the order is the dependency.** The door comes first, because every tree task
authors Pacts. The verbs come before the Keystones, which reuse the burst. The mechanisms come before
the trees that name them. The offer past a full tree comes last, since before the 27 it would rarely
fire.
...
| M7-00d | Specs for the eighty-one nodes — eleven specs where the table predicted one title | S | M7-00c | ☑ |
...
| M7-04a | [A Pact on every node, the Actives included](tasks/M7-04a-a-pact-on-every-node.md) — [row 10](#carry-forward-into-m7) | S | M7-03d | ☐ |
| M7-04b | [A burst around you](tasks/M7-04b-a-burst-around-you.md) | M | M7-04a | ☐ |
| M7-04c | [A buff that runs out](tasks/M7-04c-a-buff-that-runs-out.md) | S | M7-04b | ☐ |
| M7-04d | [An Upgrade that adds to a cast, and a clock for every skill](tasks/M7-04d-an-upgrade-that-adds-to-a-cast.md) | M | M7-04c | ☐ |
| M7-04e | [What the Oathbound's Keystones need](tasks/M7-04e-the-oathbounds-keystones.md) | M | M7-04d | ☐ |
| M7-04f | [What the Gravecaller's Keystones need](tasks/M7-04f-the-gravecallers-keystones.md) | M | M7-04e | ☐ |
| M7-04g | [What the Emberwright's Keystones need](tasks/M7-04g-the-emberwrights-keystones.md) | M | M7-04f | ☐ |
| M7-04h | [The Oathbound's twenty-seven, and the two screens a full tree outgrows](tasks/M7-04h-the-oathbounds-twenty-seven.md) — [rows 2, 3, 10](#carry-forward-into-m7) | M | M7-04g | ☐ |
| M7-04i | [The Gravecaller's twenty-seven](tasks/M7-04i-the-gravecallers-twenty-seven.md) — [rows 2, 10](#carry-forward-into-m7) | S | M7-04h | ☐ |
| M7-04j | [The Emberwright's twenty-seven, and the eighty-one counted](tasks/M7-04j-the-emberwrights-twenty-seven.md) — [rows 2, 10](#carry-forward-into-m7) | S | M7-04i | ☐ |
| M7-04k | [A full tree still tempts](tasks/M7-04k-a-full-tree-still-tempts.md) — [row 10](#carry-forward-into-m7) | M | M7-04j | ☐ |
| M7-05 | Ashen Reach: 8–12 arenas + art *(00e)* | — | M7-04k | ☐ |
...
the same Sanctum as any other. **Re-read again at [M7-00d](tasks/M7-04a-a-pact-on-every-node.md)'s
group: row 10 changed owner** — M7-04a and M7-04h to M7-04j for coverage, and M7-04k for the full
tree. Rows 2 and 3 gain what the eighty-one do to them. Rows 1, 9, 11 and 12 are unchanged.
...
| 10 | **A Pact is taken when it is seen, and it is almost never seen.** One Pact card in **62 offers** across five runs, taken the one time. **4 of 36 nodes carry a Pact and the Emberwright's twelve carry none**, and a finished tree rolls no offer at all — so GD §13.2's *"continuous temptation"* is about 2 % of level-ups, and zero after stage ~13. CH §3.1's new ¾ budget also asks for Keen Censer and Zealotry to be re-read. **Specced at M7-00d, in full.** Three things were in the way. **The rate a player sees is `PactChance` × coverage.** [M7-04a](tasks/M7-04a-a-pact-on-every-node.md) writes a Pact for every shipped node, and [M7-04h](tasks/M7-04h-the-oathbounds-twenty-seven.md) to [M7-04j](tasks/M7-04j-the-emberwrights-twenty-seven.md) for every new one: **72 of 81**, every node but a Keystone. That is one offer in four. **The Active refusal needed no second door.** A cooldown Pact lands through `ModifySkillCooldownHandler`'s pending table (M7-04a rule 1), and the ¾ re-read is M7-04a rule 4. **A full tree still offers one**: the corrupted form of a node already owned, after the Overflow is paid ([M7-04k](tasks/M7-04k-a-full-tree-still-tempts.md)).  Counting also found that **no class sweep reads a Pact's effects**, which M7-04a rule 5 closes before any class-shaped Pact exists. | **[M7-04a](tasks/M7-04a-a-pact-on-every-node.md)**, then **M7-04h to M7-04j** for coverage and **[M7-04k](tasks/M7-04k-a-full-tree-still-tempts.md)** past a full tree. The row leaves at M7-04k's *As built*. The **instrument** in the suite is M7-04a's `Offers_APactIsOneOfferInFour`; in play, **M7-08** counts offers rolled against Pact cards shown and taken | **Medium**: the signature system is invisible. |
...
- **A tree editor for `SkillTreeDefinition`.** Three nested arrays in the default Inspector is enough
  for twelve nodes. **Its promoter fired at M7-00d and the answer is no, counted:** a 27-node tree is
  three branches of five short arrays of references, and M7-04h to M7-04j each author fifteen nodes
  into one. Promoted by the first designer's complaint, or by a fourth class.
...
- **Restless Dead is a `StatTarget.Minions` node filed in the Gravecaller's *Rot* branch rather than *Legion*.** Found at [M5-08](tasks/M5-08-acceptance-and-tag.md) while confirming [M6 row 6](#carry-forward-into-m6): it is why an Oathbound borrowing the Gravecaller loses **two** of three branches instead of one, since `SplashFlow` refuses any branch carrying a minion effect. *Legion* being closed to a minionless class is inherent and correct; *Rot* being closed is an accident of where one node was filed. Deliberately not moved at M5-08 — shifting it makes Legion five nodes and Rot three, and re-balancing a branch is authoring rather than acceptance. **Placed where it is at M7-00d** ([M7-04i](tasks/M7-04i-the-gravecallers-twenty-seven.md) rule 3). `SkillTree.Restore` enforces the gates on a saved take order, so moved to Legion it would strand every save that took it after a Rot node and before a Legion one. Rot's four new nodes name no Wight, so the fix is now a splash that **drops** a node a borrower cannot use, the way it drops the Keystone, rather than refusing the branch. Promoted by the owner wanting Rot lent to a minionless class, or by a playtest that asks for it.
...
- ~~**The PlayMode suite writes the owner's real `run.json`.**~~ Promoted at [RS-03f](tasks/RS-03f-the-descend-row.md) rule 6, the next task to touch `BootSmokeTests`, to [RS-03g](#rs--the-ranger-a-side-track-the-model-first). Found at [M6-11c](tasks/M6-11c-stage-one-without-the-m1-dummies.md#as-built).
- ~~**The Sanctum still sells a Reroll when every untaken node is unreachable.**~~ Promoted at M7-00d to [M7-04h](tasks/M7-04h-the-oathbounds-twenty-seven.md) rule 8 — `SkillTree.CanStillBeOffered` — by the promoter this line named.
- **A saved run the tree refuses strands the player.** `SkillTree.Restore`'s throw — a node this build no longer ships, or a take order that breaks a gate — leaves `RunSession.Start` through `RunTicker.Start`, which has no catch. The Run scene sits idle with no message, `run.json` is kept, and Continue offers the same dead run on every launch until the player taps Descend. Found at M7-00d; M7-04h keeps every shipped node where it is so no content change reaches it. Promoted by the first change that must move or delete a shipped node, or by **M8-06**.
- **Nothing hurts a Wight.** `MinionSystem.ApplyDamage` has no caller and no enemy targets one, so a Wight's `MaxHp` is read by nothing. [M7-04f](tasks/M7-04f-the-gravecallers-keystones.md) re-reads The Host's *"50 % HP"* as −35 % damage, and [M7-04i](tasks/M7-04i-the-gravecallers-twenty-seven.md) re-aims Knitted Bone at the lifespan. Promoted by a task that lets an enemy choose a Wight; that task puts both back on hit points.

...
| **M5** | **Second class** | Gravecaller: projectile weapon + leading, Shroudstep, Wights, its tree, class select | 11 |
...
**Two corrections this table carries, both found at M4-07.** `M5-07a` has been in it since M3-00c and was never counted, so M5's task count was **8** against a table of nine rows; with the two spec tasks it is **11**. And **M5-02** is the standing owner of the **Oathbound speed-band contradiction** — GD §6.1's 5.4–6.2 m/s band against the shipped 3 m/s — by the owner's ruling re-stated at M3-15 and again at M4-07; it is the first task that has to author a second class's speed and therefore the first that cannot avoid the band.
...
- **`TreeRules` and `SkillTree` address a branch by index into one tree, and CH §5.4 needs a second
  ...
  of the two things that forced the group, and **M5-00a** is the task that writes M5-07a's spec.

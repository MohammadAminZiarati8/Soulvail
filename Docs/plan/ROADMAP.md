...
| 8 | ~~**Views and `RunTicker`'s frame order have no automated coverage at all.**~~ **The pooled-view half is closed by [M2-09](tasks/M2-09-projectile-views-and-pooling.md):** `Tests/PlayMode/PoolingLifecycleTests.cs` takes both families through a full life — rent → bind → hurt → telegraph → kill → four frames of dissolve → return → re-rent — and asserts scale, material, tint, alpha, collider, id and velocity against a body the pool has never handed out. **It had to be PlayMode by construction**, which is why it was never written by accident: `EnemyHitFeedback` captures what "clean" means in `Awake` and the dissolve advances in `Update`, so an EditMode test of `OnDespawn` asserts that nothing was undone because nothing had happened. **What is left is the frame order**, which still has no fixture at all — nothing in the project constructs a `RunTicker`, so M2-09's constructor grew a sixteenth argument with no test to update, and its own `Ticker_StepsWithSnapshotDt` row is asserted at the view instead (M2-09's *As built*, deviation 4). | ~~M2-09~~ · **M2-11b** | Medium. Halved: the pooled family is covered and grows by assertion rather than by file, and M2-11b is still the task that adds a step to the order and so makes asserting it worth one. |

...
| RS-03f | `BootSmokeTests.Descend_StartsARun_AndRefusesASecondTap` taps the Menu's first button, which is Continue when a run is on disk: green only with a save, red on a fresh install ([RS-03e](tasks/RS-03e-the-rangers-acceptance.md#as-built)) *(title)* | S | — | ☐ |

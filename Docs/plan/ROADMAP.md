...
| **M3** | **Levelling and the tree** | XP, tree rules, offers, level-up screen, SkillRunner + auto-cast, effect primitives, first Oathbound nodes, health-bar treatment | 31 |
...
**The three below are titles until 00d writes them** — no sizes, no dependencies, no boxes, because a size is a claim about a Files table that does not exist yet. **M3-13 grew twice**: M3-00b gave it a fourth palette reader and **M3-00c gave it four more — eight across five files** (`OfferCard`, `TreeNodeView`, `AutoCastRow`, `ZoneView`, `BulwarkView`), so ledger row 6 is no longer a small tidy-up. **M3-13 also inherits a readout that did not exist when it was titled**: `RunState.PlayerGrantedShield` (M3-11a), which GD §16.2's health-bar treatment is the natural home for. **M3-15 inherits the whole of row 9** — twenty-four unresolved `LocKey`s across four screens, and one of them (M3-10b's 24 dp cells) that M6-10 cannot fix because there is nowhere to put text.
...
- **Icons for the twelve nodes.** [M3-10b](tasks/M3-10b-progression-hud.md) rule 8's 24 dp auto-cast
  ...
  says the row is unreadable without them.

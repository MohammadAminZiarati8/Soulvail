...
| **M6** | **Systems complete** | Sanctum shop, Veilrot + Pacts + Claiming, Ordeals, Emberwright, unlocks, localisation tables | 19 |
...
M6-00a and to **nineteen** at M6-00b, and will count higher, so the groups are themed rather than
halved:
...
- **GD §13.2's Pact table and GD §13.2's own worked examples describe two different mechanics.**
  ...
  **downsides**, which no multiplication of a clean node can produce, and which are not 1.8× of the
  ...
  lose their downsides is the owner's call. Promoted by that ruling, or by **M7-04**, which authors
  eighty-one nodes against whichever it is.
...
- **GD §13.4's Echo admits two readings with opposite signs, and the wave curve makes it one wave in
  ...
  **and** P1's readability, is doubled. Choosing is a design decision; promoted by the owner's
  ...
  `WaveComposer` and arrives with the budget arithmetic open.

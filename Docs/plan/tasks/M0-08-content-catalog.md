# M0-08 — `ContentId`, `LocKey`, `CharacterSpec`, `ContentCatalog`

**Size:** M · **Depends on:** M0-07 · **Branch:** `m0-08-content-catalog`
**Design refs:** AR §10.1, §11.3, §11.5; ADR-0006, ADR-0010, ADR-0012

## Goal

Core has stable identities for content and text, an immutable class spec, and a catalog to look them up — so nothing in core ever references an asset or a raw string.

## Files

| Path (under `Assets/_Project/`) | Assembly | Purpose |
|---|---|---|
| `Core/Content/ContentId.cs` | Core | Validated, stable content identity |
| `Core/Content/LocKey.cs` | Core | Localisation key |
| `Core/Content/CharacterSpec.cs` | Core | Immutable class definition |
| `Core/Content/ContentCatalog.cs` | Core | Lookup by id, duplicates rejected |
| `Tests/Core/Content/ContentTests.cs` | Tests.Core | All four types (one module, one test file) — **already exists**; M0-07 created it for `MovementSpec`, so this task adds to it rather than creating it |

## Public API

```csharp
namespace Soulvail.Core.Content;

public readonly struct ContentId : IEquatable<ContentId>
{
    public string Value { get; }
    public ContentId(string value);                        // ArgumentException if invalid
    public static bool TryParse(string value, out ContentId id);
    public static bool IsValid(string value);              // ^[a-z0-9]+(\.[a-z0-9_-]+)+$
    // Equals, GetHashCode, ToString (returns Value), ==, !=
}

public readonly struct LocKey : IEquatable<LocKey>
{
    public string Key { get; }
    public LocKey(string key);                             // ArgumentException if null/empty/whitespace-containing
    // Equals, GetHashCode, ToString, ==, !=
}

public sealed class CharacterSpec
{
    public ContentId    Id       { get; }
    public LocKey       NameKey  { get; }
    public float        MaxHp    { get; }
    public MovementSpec Movement { get; }
    public CharacterSpec(ContentId id, LocKey nameKey, float maxHp, MovementSpec movement);   // maxHp > 0, movement non-null
}

public sealed class ContentCatalog
{
    public ContentCatalog(IReadOnlyList<CharacterSpec> characters);   // ArgumentException on duplicate id, listing it
    public IReadOnlyList<CharacterSpec> Characters { get; }
    public CharacterSpec Character(ContentId id);                     // KeyNotFoundException naming the id
    public bool TryGetCharacter(ContentId id, out CharacterSpec spec);
}
```

## Behaviour

1. A valid `ContentId` is lowercase, dot-separated, at least two segments: `character.oathbound`, `skill.oathbound.consecrate`. Segments after the first may contain `_` and `-`. Anything else — uppercase, spaces, leading/trailing dot, single segment, empty — is rejected.
2. `ContentId` equality is ordinal on `Value`. Default (`default(ContentId)`) has `Value == null` and is not equal to any valid id; using it in the catalog throws `KeyNotFoundException` like any unknown id.
3. `LocKey` accepts any non-empty string without whitespace. It is *not* validated against a table — that's the localiser's job (M6).
4. `CharacterSpec` is immutable and validates `maxHp > 0` and `movement != null`.
5. The catalog copies the input list; later mutation of the caller's list has no effect.
6. Duplicate ids in the constructor throw with the duplicate id in the message.
7. Lookups are O(1) (dictionary keyed by `ContentId`).

## Tests

| Test | Given / When / Then |
|---|---|
| `ContentId_ValidForms_Accepted` | `"a.b"`, `"character.oathbound"`, `"skill.x.y_z-1"` / new / no throw, `Value` preserved |
| `ContentId_InvalidForms_Rejected` | `""`, `"single"`, `"Has.Upper"`, `"a b.c"`, `".a.b"`, `"a.b."`, `"a..b"`, null / new / `ArgumentException` |
| `ContentId_TryParse_MatchesIsValid` | same inputs / TryParse / true iff valid |
| `ContentId_Equality_IsOrdinal` | `"a.b"` twice / == / true; `"a.b"` vs `"a.c"` / false; hash codes equal for equal ids |
| `ContentId_Default_NotEqualToAny` | default vs `"a.b"` / == / false |
| `LocKey_Valid_Accepted` | `"character.oathbound.name"` / new / ok |
| `LocKey_Invalid_Rejected` | `""`, `"has space"`, null / new / `ArgumentException` |
| `CharacterSpec_InvalidHp_Throws` | maxHp 0 / new / `ArgumentOutOfRangeException` |
| `CharacterSpec_NullMovement_Throws` | movement null / new / `ArgumentNullException` |
| `Catalog_LooksUpById` | one spec / Character(id) / same instance |
| `Catalog_UnknownId_Throws_NamingId` | empty catalog / Character("x.y") / `KeyNotFoundException`, message contains `x.y` |
| `Catalog_TryGet_FalseForUnknown` | — / TryGetCharacter / false, out null |
| `Catalog_DuplicateId_Throws_NamingId` | two specs same id / new / `ArgumentException` containing the id |
| `Catalog_CopiesInput` | list / construct, then list.Clear() / `Characters.Count` unchanged |

## Acceptance

- [ ] All tests green
- [ ] Zero errors, zero new analyzer warnings
- [ ] `PROGRESS.md` entry appended; Current State updated; ROADMAP box ticked

## Out of scope

- `TagSet` (ADR-0010) — first needed by M1-06 enemies.
- Enemy, skill, mode specs — their milestones.
- Building the catalog from ScriptableObjects — M0-11.

## As built

Five files as specced (four new, one added to) and 19 tests, not 14. The suite is 95, up from 76.

1. **`ContentId` rejects null as a plain `ArgumentException`, not `ArgumentNullException`.** The Tests table's `ContentId_InvalidForms_Rejected` row lists null alongside `""` and `"a..b"` and expects `ArgumentException` — and NUnit's `Assert.Throws<T>` is an *exact* type match, so a conventionally "better" `ArgumentNullException` would redden a correct constructor. Kept the spec's exception, because it is also the more honest one: there is a single question here — is this a well-formed id — and null is one of the ways to fail it, not a different kind of failure.
2. **`ToString()` returns `Value ?? string.Empty`**, where the Public API says "returns `Value`". Identical for every id that exists; the difference is only `default(ContentId)`, and a `ToString` that can return null is a trap for every caller that concatenates it. The message a default id belongs in is already saying something was not found.
3. **`CharacterSpec` rejects `default(ContentId)` — a guard beyond rule 4, with its own row (`CharacterSpec_DefaultId_Throws`).** Rule 2 promises that a default id used in the catalog misses like any unknown id. That is only unconditionally true if no spec can be *registered* under one; otherwise the catalog would hold an entry whose id `Character()` reports as missing. Guarded where the data is built rather than where it is read, so M0-11's SO conversion and every later spec type inherit it.
4. **`ContentId.IsValid` is a hand-written scanner, not a `Regex`.** The grammar in the Public API is exactly what it implements, and it is repeated verbatim in the type's remarks so the two can be checked against each other. A regex here would need either a static cached `Regex` — in a project that bans static state — or a fresh parse and match allocation per id; the scanner is fifteen lines and allocates nothing. M0-11 validates a whole catalog of authored assets through it.
5. **Five tests beyond the Tests table.** `CharacterSpec_StoresValues` (transposition, per M0-07's lesson — see *Learned* in PROGRESS for the wrinkle), `CharacterSpec_DefaultId_Throws` and `Catalog_NullInput_Throws` (deviations 3 and 6), `Catalog_DefaultId_IsUnknown` (the second half of rule 2, which no row covered), `LocKey_Equality_IsOrdinal` (`Equals`/`GetHashCode`/`==`/`!=` are in the Public API with no row at all).
6. **`ContentCatalog` throws `ArgumentNullException` on a null list and `ArgumentException` on a null element.** Conventional guards, neither in the Public API — the same shape as M0-07's null-spec guard, but given a row here because a catalog built from a partly-unassigned authoring list is a plausible M0-11 mistake rather than a programming error.
7. **`Characters` is a `ReadOnlyCollection<CharacterSpec>`, not the copied array typed as `IReadOnlyList`.** An array handed out as `IReadOnlyList<T>` casts straight back to `CharacterSpec[]`, and then the copy rule 5 asks for protects nothing. One wrapper object, once, for the life of the app.
8. **Extra assertions inside specced rows**, none of them new rows: `ToString` folded into `ContentId_ValidForms_Accepted`; the message-names-the-value check into `ContentId_InvalidForms_Rejected`, along with `a_b.c` and `a-b.c`, which are the half of the grammar (`_` and `-` are illegal in the *first* segment) that the specced invalid forms do not reach; NaN into `CharacterSpec_InvalidHp_Throws`; tab, newline, all-space and trailing-space into `LocKey_Invalid_Rejected`.

...
- **The resumed-run payload.** It is the one field M2-14b adds, and its name is `RunSnapshot Restore`; the DTO does not exist until M2-13a, so nothing is stubbed for it. The shape is stated here so M2-14b does not re-argue it — *reshape once* means these five fields are right, not that a sixth is forbidden.
...
- `PendingRun.Clear()` still has no caller (ledger row 10) — M2-14b.

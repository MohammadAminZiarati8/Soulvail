...
Built as specified in shape: an arena is a prefab full of named places, a pool raises one at a time, and `ModeSpec.ArenaFor` derives which. **Nine deviations**, two of which change a decision.

1. **`ArenaFor` walks the stages from 1 rather than hashing once.** The spec's rule — *"stepped forward by one if it lands on the arena the previous stage used"* — cannot be evaluated in O(1), because the previous stage's arena may itself have been stepped. Comparing against the previous stage's **raw** index instead is O(1) and *wrong*: with a roster of two, raw indices 0, 0, 1 give 0, 1, 1 — a repeat, at the one place the rule exists to prevent one. So the chain is replayed from stage 1, which is one integer hash per stage below the one asked about, twice per boundary and never per frame. `ArenaFor_NeverRepeatsConsecutively` walks twenty stages against a roster of two, which is the strictest form of the rule. The cost is stated in the method's own remarks.
2. **The core-side arena validation in `RunSession.Start` was not added** (rule 4's first half). Arenas are not catalog content — there is no `ArenaSpec`, and the spec's Files table does not add one — so core has nothing to resolve an arena id *against*. The check lives where the prefabs are: `ArenaPool` throws a `KeyNotFoundException` naming the id and the scene's roster, which for the **opening** stage is the run's first frame exactly as rule 4 wants. `ArenaViewTests.Descent_EveryArenaItRostersIsShipped` is what stops a later stage's arena ever being the one that is missing. This is the spec's own `Arrived_UnknownId_Throws` row, and the deviation is only that no second check was added upstream of it.
3. **`ArenaPool` is registered in `RunScope`, not `RunInstaller`.** Two of its five arguments are references to *this scene* — the prefab list and the root its bodies are parented under — and the static installer is deliberately the half a headless test can build. `EnemyViews` and `ProjectileViews` are registered there for exactly that reason, and this is the same shape as both.
4. **`WorldSnapshot` gained `SpawnPoints`**, which the Files table does not list. It is the only channel core has: rule 6 says the flow reads the points *"off `ArenaPool` through the snapshot"*, and a snapshot with nothing on it to read is not that. Carried by reference to the arena's own converted list, rebound every frame like the gate, and dropped to an empty list by `Clear` for the gate's reason.
5. **`PlayerView` gained `Teleport(Vector3)`**, also unlisted. Rule 6 requires the controller to be switched off around the write, and the `CharacterController` belongs to that view — reaching it from `ArenaPool` would be a second owner of the body.
6. **`SpawnDirector` copies the points at `Begin` rather than holding them.** The spec says `Begin` "takes" them; they now arrive on a snapshot buffer somebody else refills every frame, and M2-10's lesson — *nobody may recompose a plan a director is still holding* — is exactly the failure a held reference would set up. The copy grows and is never shrunk, so a run allocates at most once for it.
7. **`ArenaView.OnValidate` warns rather than errors** (rule 2). An error fails every test that builds an arena fixture — Unity's test framework treats an unexpected `LogError` as a failure — and the three `*Definition` ScriptableObjects already use a warning for exactly this job. It is also silent for an arena nothing has been dressed on yet, because the first moment it runs is the frame the component is added. The hard assertions are `ArenaViewTests`, which run the same `DescribeFaults` against the shipped prefabs.
8. **A second test file, `Tests/Game/Arena/ArenaViewTests.cs`.** The validation rows are about the *assets*; folding them into the pool's fixture would make an arena that broke GD §7.2 read as a pool bug. Four counted files rather than three, still size M.
9. **`Run.unity`'s dressed-in arena and gate are deleted, not kept.** Rule 6 puts every arena at the same world origin, so a grey box left standing in the scene would sit inside whichever arena the pool raised. The scene now carries an empty `Arenas` root and nothing else of the room. `DebugOverlay` lost its `SpawnPlan` dependency with the points, and `InstallerTests`' null-by-name row moved from the gate `Transform` to the arena root.

**Two smaller notes.** `Arena_Pillars` was **not** re-baked: its `NavMeshSurface` collects *physics colliders*, and the door and barrier slabs carry none, so the M1-19 bake is still true of it. `Arena_Tiered` was baked before its door was added, for the same reason made visible. And `ModeSpec`'s new `arenas` parameter is optional — null and empty mean the same thing — so no existing fixture had to change to keep compiling.

**Ledger row 13's layer half is done and the row stays**, as the spec asks: the pillars are on `Cover`, and M2-11b's *As built* is what retires the row.

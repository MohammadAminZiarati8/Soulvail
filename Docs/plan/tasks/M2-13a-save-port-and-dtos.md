...
Built as specified in shape: two versioned `readonly struct`s, one async port, one fake, and two members on `IRandom`. Not a byte of file I/O, and nothing calls any of it. **Eleven deviations, three that change a decision** (1, 5 and 6).

**1. Six of the test rows live in `Tests/Game/Adapters/SeededRandomTests.cs`, not in `Tests/Core/Save/SaveDtoTests.cs` — and the Files table could not have held them.** `Capture_Restore_ReplaysTheSameDraws`, `Capture_CoversEveryStream`, `Restore_IsCompleteAcrossAFreshGenerator`, `Restore_UnderADifferentSeed_DoesNotThrow`, `Capture_AllocatesNothing` and `Restore_AllocatesNothing` are all statements about `SeededRandom`, which lives in `Soulvail.Game` — and `Soulvail.Tests.Core` does not reference `Soulvail.Game` and must not, since that is the assembly boundary AR §2 is made of. A core test literally cannot name the type these rows are about. They went into the fixture that already owns that adapter, which is also where `SeededRandomTests.Draw` and `StreamsOf` already were. **The general rule, worth having: a spec that puts a row about an adapter in a `Tests.Core` file has mis-filed it, and the compiler says so on the first build.** `SaveDtoTests` keeps the rows that are about the *shape* — `RandomState_RecordsFiveStreams`, `Random_StreamHasNoSettableState` — which are core-side by construction.

**2. `Tests/Core/Fakes/FixedRandomTests.cs` gained one row**, `FixedRandom_CaptureRestore_ReplaysTheScript`. The fake gained two members through the spec's own ripple line, and this project's standing rule — written into `FixedClockTests`' own summary — is that an unverified fake makes every claim resting on it vacuous. M2-14a and M2-14b will script a resume through this.

**3. There was a third `IRandom` implementer the Files table did not name, and the compiler found it:** `CountingRandom`, a private nested fake inside `Tests/Core/Director/SpawnDirectorTests.cs` that wraps a real generator and counts draws per stream. It now forwards both members to the wrapped generator and **counts neither** — a capture is a read of where a stream stands, not a draw from it, so counting one would make that fixture's "the Spawn stream and no other" answer a different question. It also had to keep the `inner` reference it previously used only in its constructor. This is exactly the ripple row's "the compiler enumerates them", but it is a third test file changed rather than one.

**4. `SeededRandom`'s five stream properties became expression-bodied reads over five `Pcg32` fields.** `Capture` needs the concrete generator and an `IRandomStream`-typed auto-property cannot give it one without a cast. No behaviour changed and no sequence moved; `SameSeed_SameSequence_EveryStream` is the row that says so.

**5. `FixedRandom`'s `ScriptedStream.Position` clamps a `ulong` into `int` on the way in, and that is a decision about what a captured state *means* to a fake.** A scripted stream's position is how far down its list it has read, so a `RandomState` that came from this fake round-trips exactly — but one captured from a real `SeededRandom` holds a 64-bit LCG state word, and handing that to this fake should exhaust the script rather than overflow into a negative index. Clamped to `int.MaxValue`, so the failure is "the script ran out", which is the behaviour the fake already documents.

**6. `InMemorySaveStore.FailNextWrite` arms `ClearRun` as well as the two saves.** Rule 13 says "the next write"; a clear is a write to the store in every medium that has one — it is a file delete on the local adapter and a request on a syncing one — and a fake that could not fail it would leave M2-14b's "the run was cleared" path with no failure case at all. One write only, whichever of the three comes first.

**7. Write counts count *calls*, not successes:** a write armed by `FailNextWrite` still increments. What a test asserting on these wants to know is how often the run reached for the store, and a counter that skipped failures would make "a failed write did not end the run" unprovable from the count. Documented on the property and pinned by the last line of `Fake_FailNextWrite_FaultsTheTask`.

**8. AR §6's `IRandom` row was annotated too, not only §10.3's `ISaveStore` block.** The Files table named §10.3. §6 is the ports contract, and after this task its `IRandom` row — "Named streams: `Spawn`, `Offers`, …" — was an incomplete description of the port. One clause, naming where the pair lives and where it deliberately does not. §10.3 also gained the `RandomState` line and the `in`-parameter line, because both are rules a reader of that block needs and neither was inferable from the signatures.

**9. Two guard rows cover more than their names promise.** `Snapshot_NegativeHp_Throws` checks `playerShield` as well, and `Snapshot_NegativeRunTime_Throws` checks NaN and infinity as well. The guards are identical in shape — AR §18.3's `!(v >= 0f) || float.IsInfinity(v)`, three times — and six rows would have said one thing six times. `Snapshot_NonFiniteHp_Throws` is kept separate because *which* spelling admits NaN is the thing worth a row of its own.

**10. `SaveDtoTests`' `Snapshot` helper does not parameterise the two `ContentId`s, so `Snapshot_DefaultContentIdIsAllowed` builds its snapshot directly.** An optional `ContentId?` parameter cannot tell "not supplied" from "supplied as `default(ContentId)`" — both arrive as null — and that value is precisely what the row is about. The row that would have been silently vacuous is the row that names the trap.

**11. `InMemorySaveStoreTests` is the first fixture in the suite with `async Task` test methods.** Nothing else in 961 rows awaits anything, because nothing else in the project was async. NUnit runs them and the pass count confirms it; no `[UnityTest]` or coroutine was needed, since every task the fake returns is already completed.

**Verified:** **960 passed, 1 failed, 0 skipped, 13.3 s** — 931 at M2-12b plus **30 new rows**, every one of them green. Zero compile errors, zero new analyzer warnings, `git diff ProjectSettings/` empty, no asset or scene touched. **The one failure is not this task's and is not new:** `ModeDefinitionTests.Descent_MatchesDesign` fails on `StartingStage` being 5 where GD §8.2 says 1, because `Descent.asset` carries `_startingStage: 5` **on `dev`** — committed in `ea0627f` (M2-12b), which is the playtest workaround PROGRESS had recorded as over. One line in one asset, outside this task's Files table and squarely the owner's knob. Raised in PROGRESS → Known issues.

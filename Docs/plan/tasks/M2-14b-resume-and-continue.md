...
**Built as specified, with eight deviations. Two change a decision** — the spec's `Menu_ContinueSetsThePendingRun` and `Boot_LoadsBeforeLeavingBoot` could not be written as worded, because `SceneLoader` is `sealed` with no port behind it.

**Verified:** 1077 EditMode passed, 0 failed, 0 skipped, 18.5 s — and 1077 again at 15.2 s, run twice. M2-14a's 1035 plus **42 new rows, every one green**: sixteen in `RunSessionResumeTests`, six in `SavedRunTests`, twenty in `ResumeFlowTests`. Zero compile errors, zero new analyzer warnings; the only two Console warnings are the pre-existing `ArenaView.OnValidate` import-order pair. `git diff ProjectSettings/` **empty**. `Menu.unity` is **260 insertions and 0 deletions** — a pure addition, no collateral re-serialisation (Traps §5).

**Ledger rows 1 and 10 both leave the table here.** Row 1's restore half is `RunInstaller.CreateRandom` calling `IRandom.Restore` on the generator it just built, proven end to end by `Installer_RestoresTheGeneratorState` (the 501st draw) and `Resume_ComposesTheSameStage` (two runs into stage 4, bodies compared entry for entry). Row 10's caller is `RunTicker.Start`, after `_session.Start` returns, proven by `Ticker_ClearsAfterTheSessionStarted`.

### Deviations

1. **`Core/Combat/Health.cs` was edited, and it is not in the Files table.** It grew `internal void Restore(float hp, float shield)` and a private `Clamp`. Nothing in `Health`'s existing surface can land on a stated HP-and-shield pair: `Reset()` goes to full, `Heal` only rises, and `ApplyDamage` spends the shield *before* it touches HP — so one call cannot reach (62, 9) — besides starting i-frames and holding off the shield's refill, which would give a resumed run a beat of invulnerability it did not earn. `Health` owns both fields privately, so it is the only honest home. `internal`, so the Unity side cannot reach it and `Soulvail.Tests.Core` (no `InternalsVisibleTo`) tests it through `RunSession.Start` like everything else. Both values are clamped rather than refused, because `RunSnapshot`'s constructor already refused negatives and non-finite values at the real boundary; what is left is a save whose numbers were legal under a *different* maximum, which is M3's migration problem and not a reason to refuse the run.
2. **The sweep was 38 call sites, not 19 — and the 38th was invisible to grep.** `ProjectileSystemTests.cs:709` constructs its config as target-typed `new(...)`, so a search for `new RunConfig` cannot see it. This is [Traps §2](../../Traps.md)'s own warning — *"a `grep` for `new TypeName` does not find every construction … let the compiler enumerate the call sites"* — and the compiler is exactly what found it. The other 37 were swept mechanically by matching parentheses rather than by `sed`, because the call sites span up to five lines each.
3. **`Menu_ContinueSetsThePendingRun` does not tap the button.** It asserts `PendingRun.Resume(savedRun.Value)` — the one statement the handler runs before it awaits anything — rather than invoking `onClick`. `MenuPresenter.Continue` calls `SceneLoader.LoadAsync(Run)`, and `SceneLoader` is `sealed` with no interface, so an EditMode test that tapped the button would call `SceneManager.LoadSceneAsync` and take the Editor's open scene with it. Rejected: making `LoadAsync` virtual or extracting an `ISceneLoader`, which is a production design change made for a test. The tap end to end is the owner's manual step 2, and `MenuScope`'s own remarks already say the Menu's behaviour is proven by playing it (M0-17).
4. **`Boot_LoadsBeforeLeavingBoot` asserts synchronous completion, not "before the Menu scene was requested".** Same root cause as 3: the hand-off cannot be observed without a fakeable loader. The row asserts `SavedRun.IsPresent` is true by the time `BootFlow.Start` **returns**, which is the property rule 6 actually rests on — `LeaveBoot` is the last statement of that same method, and the only other caller is a later `sceneLoaded`. It also carries an `Assert.Ignore` interlock for the case where the Editor's open scene *is* Boot, because `Start` would then hand off for real; it did not fire in either run (0 skipped).
5. **`MenuPresenter.Descend` was touched, where rule 7 says "`Descend` is unchanged".** It now lowers and restores `_continue.interactable` alongside its own. uGUI refuses a second touch on the button that was hit and says nothing about the one beside it, so without this a player could tap `Descend` and then `Continue` during the same scene load and start two runs. What rule 7 meant — that `Descend` does not delete the saved run — is unchanged and is asserted by `Menu_DescendStartsFresh`.
6. **`BootFlow`'s constructor gained a fourth parameter.** Named here because it is a public API change the Files table describes only as "loads the run beside M2-13b's profile". Nothing outside the container constructs one, so there was no sweep.
7. **Thirteen rows beyond the Tests table**, all of them the "implied, not listed" guards plus four that earn their place: `Resume_DoesNotRewindTheStreams` (core must *not* apply `snapshot.Random` — the row fails if anyone ever gives core the door M2-13a closed), `Start_RestoredRunTimeKeepsRunning` (simulated seconds carry on rather than restart), `Menu_VisibilityIsDecidedOnEveryEnable` (the Menu is re-entered after a death, so reading `IsPresent` once at injection would keep offering a cleared run), and `Ticker_BuildsTheFreshConfigFromTheMode`.
8. **`Resume_LandsInTheSameArena` needed no new code.** `ModeSpec.ArenaFor` was already a pure function of `(stage, seed)` — M2-11a rule 3 built it that way *for* this task — so the arena half of rule 5 is a row and nothing else. The row asserts both the pure function and the `StageArrived` the resumed run publishes, so it fails if either the derivation or the wiring moves.

**One thing found by playtest rather than by a test, and it was the guard working.** The owner played to stage 2 and died before `Menu.unity` had been given its button, and `MenuPresenter.OnEnable` threw `MissingReferenceException` naming the field and what to drag onto it. That is M0-11's scene-guard convention doing exactly its job — the scene edit was the half of the task still outstanding, and nothing about it was silent.

**Also changed, outside the Files table:** [Traps.md](../../Traps.md) §3 gained the offline-compile technique. Unity writes its real `csc` argument list to `Library/Bee/artifacts/<dag>.dag/<Assembly>.rsp`, and Roslyn ships at `Editor/Data/DotNetSdkRoslyn/csc.dll`, so all six assemblies can be compiled — with the real analyzers and `-langversion:10` — while the Editor is unfocused and refusing to drain a compile. This session spent about twenty minutes waiting on focus before doing that; the note is there so the next one does not.

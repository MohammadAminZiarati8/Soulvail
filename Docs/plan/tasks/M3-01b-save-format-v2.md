...
**Built as specced. Sixteen files, every one of them in the Files table**, and the ripple was exactly the 13 `new RunSnapshot(...)` sites across 10 files the table predicted — the first time this milestone's arithmetic about its own ripple has been right first go (M3-01a's was a third short). No new files, no assets, no `ProjectSettings/`, no scene re-serialisation.

**Three decisions the spec left to the implementation:**

1. **An empty node list costs nothing, and that is what keeps `Take_AllocatesNothing` green.** Rule 5 names the copy as an allocation at a boundary and accepts it. A copy of *nothing* is `Array.Empty<ContentId>()`, so `RunRecorder.Take` — which passes an empty list until M3-03 — still asks for no heap, and the row is left standing rather than deleted. The short-circuit is not an optimisation bolted on: the same shared empty array is what `TakenNodeIds` has to answer for `default(RunSnapshot)`, which rule 5 requires anyway. **`Take_AllocatesNothing` is therefore M3-03's row to retire**, and both the row and `RunRecorder`'s class remark say so in as many words, so that task changes a documented trade rather than discovering a red test.
2. **Each migration step writes the version it produces (a literal `2`), not `CurrentVersion`.** `MigrateRun` now walks a local `current` through the chain. Written this way for M3-07b, which adds the second step: a step that wrote `CurrentVersion` would hand the *next* step a DTO already claiming to be v3.
3. **The ctor's `default(ContentId)` guard throws `ArgumentException`, the null guard `ArgumentNullException`.** Both are already caught by `LocalJsonSaveStore.IsUnreadableSave`, so a hand-edited file with an unparseable node id is discarded like any other corrupt save rather than reaching a reader. `ToContentIds` is null-tolerant so that `"takenNodeIds":null` is an empty list rather than a `NullReferenceException` out of a load.

**Two things checked rather than assumed, because both would have failed silently on a player's device:**

- **`RunMirror.level = 1` as a field initialiser is load-bearing and is now proven by a fixture.** A v1 document has no `level` key, `JsonUtility` leaves the field at its initialised value, and `RunSnapshot` refuses a level below 1 — so without the initialiser *every v1 save already on a device* would have been discarded with "Discarding the save" before the migration ever ran. `Fixture_V1Run_DecodesToTheExpectedSnapshot` runs the real adapter over the untouched v1 literal and is what says so.
- **The v2 literal is hand-typed and `Fixture_V2Run_IsWhatThisBuildWrites` compares byte for byte**, key order following the mirror's field order with the four new keys after `writtenAt`.

**One soft spot found and deliberately not fixed here, because fixing it needs a policy the spec does not state.** `LevelTracker.Restore`'s settle loop terminates for any finite `xp` — `XpCurve.IsAuthored` guarantees a positive cost per level — but a *hand-edited* `run.json` carrying, say, `"xp":1e38` is a legal `RunSnapshot` (finite, non-negative) and would take on the order of 10^15 iterations to settle, which is a hung frame rather than a wrong number. **It is not new and it is not this task's:** `LevelTracker.Grant` has the identical exposure through a content-authored `EnemySpec.XpValue`, and M3-01a's own *Learned* note already records that the `while` loop is the one place in the project where a bad number hangs instead of misbehaving. Capping it means deciding what a save above the cap *becomes*, which is a ruling, not an implementation detail. Recorded in the ROADMAP parking lot rather than invented here.

**Two test names differ from the spec by one character:** `Recorder_NodesAreEmptyUntilM3-03` and `Start_IgnoresTakenNodesUntilM3-03` are `…M3_03` in code, because a C# method name cannot contain a hyphen. Both carry the "**M3-03 replaces/flips this row**" comment the spec asks for.

**Verified:** **1121 EditMode / 0 / 0, twice consecutively** against M3-01a's 1104 — **17 new rows**, which is exactly the spec's Tests table less the four rows that were renamed or extended in place (`Snapshot_DefaultHasEmptyNodesAndVersionZero`, `Migrate_V2_IsIdentity`, `Fixture_V2Run_IsWhatThisBuildWrites`, `Start_RestoreIsAppliedBeforeRunStarted`). Six assemblies, **zero compile errors, zero analyzer warnings**, every changed file confirmed in its intended assembly through `GetAssemblyNameFromScriptPath` rather than assumed (Traps §5). PlayMode **11/11 then 10/11** — `FrameOrderTests.Ticker_RunsTheStepsInOrder`, M2-15a's exact message, the Known-issues row returning; it drives a `RecordingCore` fake `IRunSession`, so the line this task added to `RunSession.Start` never executes in it. `git status` clean of anything unasked.

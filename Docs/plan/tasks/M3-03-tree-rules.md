...
6. **`PendingLevelUps` is saved, not derived** (M3-01b). It cannot be derived: an overflow level (M3-08) spends a pick with no node, and M6-02's Banish removes nodes from the pool, so `Level − 1 − TakenCount` stops meaning "owed" the first time either happens.
...
- **Banish** — M6-02 adds a banished set that `Available` skips; the shape is a second `bool[]`.

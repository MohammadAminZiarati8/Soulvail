...
- **Cooldowns on disk.** A resumed run comes back with every skill ready, like a resumed run comes back with the Charge ready today. **The reason first given here was wrong and was rewritten at build time** — it said saving them would need `RunState.Time` to mean something across a process death *"which it deliberately does not"*, and it does: `RunTime` is a **v1** field and `RunSession.Start` restores `State.Time` from it exactly. The real reason is cost and review size. `SkillRunner._readyAt` is keyed by the runner's registration order, which is *derived* from `TakenNodeIds`, so the only safe spelling on disk is a keyed pair of lists with its own length, duplicate and non-finite guards — not one field — and it does not belong in the PR that ships this format's first two-step chain. **The later bump is cheap, and that is a fact about the code**: `_readyAt` is absolute against a clock that is itself restored exactly, so a v4 step is add-only, which is the case `MigrateRun`'s signature already handles. **The known gap meanwhile** — quitting at a boundary and resuming returns every skill ready, so `Continue` is a free cooldown reset, bounded by the longest cooldown and by a stage's opening seconds. Ruled by the owner at build time; recorded rather than left to be discovered.
...
**Built as specced, with one deviation, two spec corrections and one finding.** `RunSnapshot` is v3 with `ManualSkillIds`, the v2 → v3 step ships beside it, and a v1 document now walks both steps in one decode. **1373 EditMode / 0 / 0, twice consecutively** (runs 2 and 3, with PlayMode between them), **PlayMode 11/11**, six assemblies, zero errors, zero new analyzer warnings.

**The test arithmetic lands to the row.** The Tests table is 24 rows; **seven already existed** and were renamed or extended in place, so the delta is **17**, and 1356 + 17 = 1373. The seven: `Chain_IsUnbrokenFromOldestToCurrent` (unchanged — it loops to `CurrentVersion` and follows the bump by itself, which is the point of having written it at v1), `Gate_AcceptsOneAndTwoRefusesThree` → `Gate_AcceptsOneToThreeRefusesFour`, `Migrate_V2_IsIdentity` → `Migrate_V3_IsIdentity` (**replaced, not joined** — identity is a property of the *current* version, so it moves up with every bump and there is only ever one such row; what it used to assert is now `Migrate_V2_GetsEmptySlots`, a step rather than an identity, which is exactly the transition a bump makes), `Migrate_V1_GetsUnlevelledDefaults` → `Migrate_V1_RunsBothStepsInOrder`, `Fixture_V1Run_…` and `Fixture_V2Run_DecodesToTheExpectedSnapshot` extended, and `Fixture_V2Run_IsWhatThisBuildWrites` → `Fixture_V3Run_…`. **The spec's own ripple number was stale**: it said "13 across 10 files at M3-01b"; it was **17 sites across 13 files**, counted before planning, and a 15-argument constructor rewritten in 13 files is most of this task's real size — **"S" is priced against the wrong number.**

**The one deviation: `SkillRunnerTests` changed and is not in the Files table.** M3-07a's pin `Snapshot_CarriesNoLoadout` asserted `CurrentVersion == 2` and walked `RunSnapshot`'s members refusing any name containing `"manual"`, so it went red twice over — the red row this task existed to change. It is **inverted in place as `Snapshot_CarriesTheLoadout`** rather than deleted, because the thing worth asserting for ever is the same claim from the other side: the runner's table is what reaches the file. Net zero rows. The file was already in the ripple set; the pin is a behaviour change rather than a mechanical edit, hence named here.

**The non-finite guard row is not owed and that is deliberate.** v3 adds no `float` door — `ManualSkillIds` is a list of `ContentId` — so the implied-guard rule's third clause has nothing to attach to. Said out loud rather than left as an absence, which is the omission M3-04's *As built* was pulled up on. The null and validation clauses **are** owed and are paid: `Snapshot_NullSlots_Throws`, `Snapshot_WrongSlotCount_Throws`, `Snapshot_DuplicateSlotEntry_Throws`.

**`internal Restore` survived, unlike M3-03's.** Checked row by row rather than assumed: all five restore rows drive `StartResumed`, and `RunSession` is in `Soulvail.Core` beside `SkillRunner`, so nothing needs the direct call that forced M3-03's to go public. `Soulvail.Tests.Core` still has no `InternalsVisibleTo` and still never will (AR §18.2).

**A finding the spec did not predict: the fixed-length list would have made every boundary capture allocate**, breaking `Take_AllocatesNothing` — a row in the Files table's own file, failing for a reason that would have read as a guard bug. The list is always four long, so there is no empty case to shortcut on *length*. Fixed the way `TakenNodeIds` already solves its half: **four empty slots are indistinguishable from any other four**, so `RunSnapshot` keeps one shared immutable `EmptySlots` and a run with nothing on Manual copies nothing. The occupancy scan therefore runs **before** the array is allocated — allocating up front and returning the shared instance at the end would leave a dead four-element array behind on exactly the measured path. Every run is that run until a player opens CC §6.3's screen, so the row is still measuring a real zero rather than passing on a technicality. **The copy is not optional once a slot is filled**, and for a sharper reason than the node list's: what is passed is `SkillRunner.Slots`, a *live* view the player's next `SetAutoCast` writes through.

**A second correction, in a test fixture rather than the spec:** the three Actives the recorder rows need went into branch c's existing tier, **not a fourth branch** — `SkillTreeSpec` refuses a class with anything but three (CH §5, identical skeleton so the UI is built once). Found by two red rows, not by reading.

**Both older literals survive byte for byte.** `V2Run` was not regenerated and is now the v2 → v3 step's input, which is what earns `Fixture_V2Run_DecodesToTheExpectedSnapshot` the right to prove anything; `V1Run` is untouched and now proves a *chain* rather than a step. The new `manualSkillIds` key follows `takenNodeIds` in the mirror's field order, hand-typed like both of its predecessors, with S1 and S3 filled so the hole is in the bytes. `RunMirror.manualSkillIds` is initialised to four entries as a **field initialiser** — harder load-bearing than `level = 1`, because `RunSnapshot` refuses a list that is not exactly four, so a zero-length default would discard every pre-v3 save *before* the step that fills it ever ran.

**AR §18.1's restore row gained its fifth line and was rewritten as an order** rather than as the pair it had been since M3-03: `LevelTracker.Restore` → `SkillTree.Restore` → the runner is told the restored Actives → **`SkillRunner.Restore`** → `Health.Restore` last. Two of the four gaps are load-bearing; the others are written in anyway, because a block read as an order invites the next line to be placed anywhere.

**Ledger row 2 moved and did not close** — checked against the row's own text rather than the header. Its owners are `M3-01b` · ~~`M3-03`~~ · **`M3-07b`** · **`M3-09c`**, and a row leaves only when its *last* owner's *As built* says it is answered. M3-09c's profile v2 — the first step `MigrateProfile` will ever run — is outstanding, so the row stays open with one bump remaining. What it gained: the chain now runs **two steps in sequence on one document**, which is the shape this row has asserted since v1 and never exercised.

**Manual steps:** three, listed for the owner in the handover, and step 1 needs a hand-authored tree with an Active because M3-12 has not shipped.

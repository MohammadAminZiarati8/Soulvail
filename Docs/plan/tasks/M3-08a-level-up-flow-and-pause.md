...
- **Reroll, Banish, Vigil, Pacts** — M6-02b, M6-05b, M6-06b. AR §6 lists `Reroll()`, `Banish(skillId)`, `BuyHeal()` and `BuyCleanse()` on this port; a port grows a member when the mechanic lands, so none of the four is written here.
...
5. **`Open_BlockedButNotFull_GrantsOverflow` could not be built, and the reason is a finding rather
   ...
   **Rule 2's code is still written on `Draw`'s count rather than `IsFull`** and should stay that way —
   M6-02b's Banish removes nodes *from the pool*, which is the first thing that can create the state.

...
**`PausePresenterTests.Panel_HasResumeAndQuitOnly` is renamed `Panel_HasResumeSkillsAndQuit`** and its two counts moved to **3** and **4**, as M3-09a's own comment instructed — **with that comment's other half corrected, because it names the wrong successor**: the View Tree button is **M3-09d**'s (its Files table lists `PausePresenter` and `Pause.prefab`), and M3-09c is profile v2 and touches neither; it now also asserts `_skills` is dressed on the prefab and says in place why `_skillsScreen` deliberately is not. The three panel labels are `LocKey`s — `ui.pause.resume`, `ui.pause.skills`, `ui.pause.quit` — and the row still asserts every one starts with `ui.pause.` and holds no space. `Skills.prefab`'s own three user-facing strings are `ui.skills.empty`, `ui.skills.close` and `ui.skills.cancel`, checked the same way: **ledger row 9, ADR-0012, and this screen is deliberately not the place that breaks the run.**

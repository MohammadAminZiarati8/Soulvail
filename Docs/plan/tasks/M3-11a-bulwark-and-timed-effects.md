...
**Both halves have landed.** [M3-11a-i](M3-11a-i-granted-shield-pool.md) merged the pool on 2026-09-17 and [M3-11a-ii](M3-11a-ii-bulwark-and-timed-effects.md) merged the clock, the primitive, its handler, its Inspector half and the two events on the same day — **1 653 EditMode / 0 / 0, PlayMode 16 / 16** — so the whole of M3-11a is built and this spec is closed. **The split was worth what it cost:** the second half deviated on four points, one of which is a decision rather than a detail — `IEffectHandler<T>.Apply` is handed no clock, so the handler holds a run-scoped `SimulatedClock` written beside `State.Time += Dt` — and it would otherwise have been taken under pressure at file four of a single seven-file task. **A fifth correction to this spec surfaced there and belongs with the other four:** rule 12's test row says *“SkillCast then the grant”* and the code publishes them the other way round, because `SkillRunner.Fire` applies the cast effects and *then* announces the cast (M3-06 rule 9).

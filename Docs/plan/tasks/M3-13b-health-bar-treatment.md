...
- **The Files table was short by two test files, and the count is five rather than four.** `Tests/Game/Views/EnemyHitFeedbackTests.cs` **did not exist** — the table says it *"gains the tint rows"*, but `EnemyHitFeedback` has never had a fixture, so `Tint_KillingBlowDoesNotFlash`, called *"the existing M1-12 row"*, had to be written too. And the nine `Player_` rows had **no named home at all**: there is no `HudPresenterTests.cs` and no `HpBarViewTests.cs` in the project, and the only assertions ever made about either class are `PaletteTests.HpBar_DrawsThePaletteColours` and M3-10b's three rows in `XpBarViewTests`. `Tests/Game/Presentation/GrantedShieldTests.cs` is the third new file and the declared deviation. **Ruled out loud before a line was written:** four new code files plus one substantial rewrite is **five**, size M is 4–5, so it fits at the ceiling and did not split.
- **The pre-declared tripwire did not fire, and the swap measured it.** The tint is a third term in the existing composition — `_liveColour` keeps its one documented meaning and a single scalar `_damageTint` is composed with it by `BodyColour()` — so `SetColour` is still the only writer of `_baseColor` in the project. **Swapping to the rejected arrangement reddens exactly one row**, `Tint_ResetForgetsIt`, on a full 1 912-row suite. **The spec expected two and it is one:** `Tint_ArchetypeLookRebasesIt` stays green under *both* arrangements, because `SetArchetypeLook` assigns `_liveColour` outright either way — so it is a row about the rental being absolute, not a row that discriminates between the two shapes. Measured, not asserted.
- **`IsElite` is a defaulted parameter, and the assembly count is three rather than four.** The Files table names three files for this change; a required parameter would have edited eleven call sites in five extra test files and pulled `Tests/PlayMode/FrameOrderTests.cs` into the blast radius to restate a fact the default already states. **The cost is stated rather than left:** *"nobody set it"* and *"not an Elite"* become indistinguishable, which is harmless here and only here, because `false` **is** not-an-Elite — the same value a fully-specified call would pass, not a sentinel standing in for one. A future field whose unset value is not also its correct default does not inherit this. `Soulvail.Core`, `Soulvail.Game`, `Soulvail.Tests.Game`; PlayMode untouched, which is the ruling's whole point.
- **`Tint_NeverApproachesDanger` is written as *not all three channels within 0.15*, and the other reading is red on shipped content.** `Danger` is `(1.000, 0.290, 0.122)`; the **Bloater** ships at `(0.639, 0.341, 0.220)`, so its green is 0.051 away and its blue 0.098 away **before any tint is applied**. *"No channel within 0.15"* would therefore fail on an untouched body at full health. **Red is the entire margin** — the Bloater's green crosses `Danger`'s exactly as hp falls and its blue converges to 0.012 — and the worst red distance over all three archetypes at all 21 steps is **0.361**, 2.4× the band. `Palette.EnemyDying` is `#3B2422`, S 0.42, V 0.23; `IsDanger` is false at every sample.
- **Rule 9's *"oriented once"* rests on a false premise and was kept in intent rather than in letter.** The spec's argument is about `FollowCamera` — true — but `EnemyView.Face` writes `transform.rotation` on the **root** every tick a chaser walks, and a bar parented to the body inherits that yaw, so a rotation written at `Start` would be wrong by the first frame the enemy turned. The bar aligns in `LateUpdate`, only while shown and only when the body's rotation actually changed. `Bar_IsOrientedOnceNotPerFrame` survives verbatim for a standing body; `Bar_ReorientsWhenTheBodyTurns` and `Bar_IsNotOrientedWhileHidden` are the two rows the correction owes.
- **`EnemyView.cs` is a sixth file, and its own remarks asked for it.** `OnDespawn` says *"anything else added to the prefab that remembers something joins the list here"*, and the Elite flag is the first thing to take that up. The file gains a cached `HealthBar` property mirroring `Feedback` — the only way `EnemyViews` can bind on the rental without a `GetComponent` per spawn — and one `Unbind` call. A small additive edit; it does not count toward the size.
- **A red test found a real design bug in rule 10, and the rule changed rather than the row.** A plain clamp collapses the segment to nothing at full health — which is exactly when Bulwark grants a shield — while letting it overhang would put an 80 dp band across the `current/max` readout `HudPresenter.Place` puts 8 dp to the bar's right. So the width is preserved and the **start** gives way: the segment is ahead of the fill while there is room and pressed against the bar's right-hand end when there is not. `Player_SegmentStaysOnTheBarAtFullHealth` is the row that caught it.
- **Two small things worth knowing.** `EnemyHitFeedback.Update` and `EnemyHealthBar.Update` were split into `Tick(dt)` on `FirstActiveHint`'s precedent — `Time.deltaTime` cannot be advanced in EditMode (Traps §5), so without it every claim about what the tint looks like *after* a flash would have had to be a PlayMode row. And `OnDied` now opens the dissolve at `t = 0` instead of at one frame's worth, which is one frame of alpha nobody can see.
- **Colour equality in a test is a trap and is now documented in the fixture.** A `MaterialPropertyBlock` round-trips a channel at about one ULP — `0.431372553` in, `0.4313725` out — so `Color.Equals` is **false** on a value nothing ever changed while Unity's `operator ==` is true, and NUnit uses the former. Four rows went red on that before `AssertColour` compared per channel within `1e-4`.
- **The prefab diffs carry M3-13a's dead lines away, as predicted.** `Hud.prefab` loses `_fillColour`, `_blockedColour` and the four kind tints; `Enemy.prefab` loses `_flashColour`. Nothing else moved, and **`Run.unity` did not move at all**.
- **Flagged, not edited:** GD §16.2 says a dying body *"darkens and shifts toward red"* and GD §16.4 reserves saturated red-orange *"for nothing else, ever"*. §16.4 wins here and the contradiction is the owner's — already a parking-lot line, resolved by M3-15 rule 9.
